    ...
    try {
        ...
        foreach ($osnovne_smjene as $key => $naziv) {
            ...
            if ($vrijeme_data) {
                ...
                $smjene_rezultat[$key] = "$naziv (vrijeme nije definisano)";
            }
        }
...
function aktivne_smjene() {
    ...
    try {
        ...
        foreach ($osnovne_smjene as $key => $naziv) {
            ...
            // SAMO DODAJ AKTIVNE SMENE (one koje imaju vremena u bazi)
            if ($vrijeme_data) {
                $pocetak = date('H:i', strtotime($vrijeme_data['pocetak']));
                $kraj = date('H:i', strtotime($vrijeme_data['kraj']));
                $smjene_rezultat[$key] = "$naziv ($pocetak-$kraj)";
            }
            // NE DODAVAJ smene koje nisu aktivne
        }
        
        return $smjene_rezultat;
    } catch (Exception $e) {
        error_log("Greška pri dohvaćanju aktivnih smena: " . $e->getMessage());
        return []; // vrati prazan niz ako greška
    }
}