...
<?php if (isset($_GET['msg']) && $_GET['msg'] === 'dodan'): ?>
    <div class="alert alert-success">
        ...
        <?php if (isset($_GET['dodano'])): ?>
            Dodano: <?= (int)$_GET['dodano'] ?> raspored<?= (int)$_GET['dodano'] > 1 ? 'a' : '' ?>.
        <?php endif; ?>
    </div>
<?php elseif (isset($_GET['msg']) && $_GET['msg'] === 'dodano_delimicno'): ?>
    <div class="alert alert-success">
        <strong>Raspored je djelomično dodan.</strong><br>
        ✅ Uspešno dodano: <?= (int)($_GET['dodano'] ?? 0) ?> raspored<?= (int)($_GET['dodano'] ?? 0) > 1 ? 'a' : '' ?><br>
...
<?php elseif (isset($_GET['msg']) && $_GET['msg'] === 'duplikat'): ?>
    <div class="alert alert-warning">
        ...
        <br><small>Preskočeno: <?= (int)($_GET['preskoceno'] ?? 0) ?> duplikad<?= (int)($_GET['preskoceno'] ?? 0) > 1 ? 'a' : '' ?></small>
    </div>
<?php elseif (isset($_GET['msg']) && $_GET['msg'] === 'nista'): ?>
    <div class="alert alert-warning">Nijedan raspored nije odabran za dodavanje.</div>
